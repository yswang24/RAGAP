...
#       - Uses ONLY embeddings from current model forward on 'data' (call site must ensure data[relation].edge_index is TRAIN-only).
#       - AUC negative sampling masks ALL known positives across train∪val∪test.
#       - Ranking (MRR/Hits@K) masks ALL known positives across train∪val∪test before top-k.
#     """
#     from collections import defaultdict
#     import numpy as np
#     import torch
...
#         # Forward (embeddings for all nodes)
...
#         # ======================
#         # NEW: Global positive map (train ∪ val ∪ test) for strict masking
#         # ======================
...
#         # ======================
#         # AUC 计算（与原逻辑等价，但使用全局屏蔽 global_pos_map）
#         # ======================
#         def compute_auc_for_pairs(pairs: tuple[torch.Tensor, torch.Tensor], _unused_split_map: defaultdict[int, set[int]]) -> float:
#             src_cpu, dst_cpu = pairs
#             if src_cpu.numel() == 0:
#                 return float('nan')
...
#                 # ==== STRICT: mask ALL known positives across train∪val∪test ====
...
#                     # ==== STRICT: 屏蔽该 phage 的所有已知正 host（train∪val∪test）====
...
#                     phage_real_id = phage_idx2id.get(ph_global, str(ph_global))
#                     true_species = {hostid2species(h) for h in ph2hosts[ph_global]}  # species-level 命中口径（保持不变）

#                     # record predictions
#                     for rank_pos, h_idx in enumerate(topk_idx, 1):
#                         host_real_id = host_idx2id.get(int(h_idx), str(int(h_idx)))
#                         host_species_name = hostid2species(int(h_idx))
#                         # 与你原先保持一致：概率 = sigmoid(score)
...
#                     # reciprocal rank（species-level 命中）
#                     rank_val = None
#                     for pos, h in enumerate(topk_idx, 1):
#                         if hostid2species(int(h)) in true_species:
#                             rank_val = pos
...
#             else:
#                 # fallback: original per-phage loop
#                 for ph_idx, true_ds in ph2hosts.items():
#                     scores_tensor = torch.sigmoid(
#                         model.decode(out, (torch.full((n_hosts,), ph_idx, device=eval_device),
#                                            torch.arange(n_hosts, device=eval_device)),
...
#                         host_real_id = host_idx2id.get(int(h), str(int(h)))
#                         host_species_name = hostid2species(int(h))
#                         score = float(row[int(h)])  # row 已是 sigmoid 后的值
#                         prediction_rows.append({
#                             "phage_id": phage_real_id,
#                             "rank": rank,
#                             "host_id": host_real_id,
...
def compute_metrics_fullgraph(
    ...
    try:
        ...
        # ======================
        # 全局正样本屏蔽表（严格 filtered setting）  先注释
        # ======================
        # def build_global_pos_map(*pairs_list):
        #     gmap = defaultdict(set)
        #     for pairs in pairs_list:
        ...
        def compute_auc_for_pairs(pairs: tuple[torch.Tensor, torch.Tensor]) -> float:
            ...
            if getattr(model, "decoder_type", None) == "cosine":
                ...
                for s_idx, d_idx in zip(src_cpu.tolist(), dst_cpu.tolist()):
                    ...
                    # 构造候选负例
                    #known = global_pos_map.get(s_idx, set())  先注释
        ...
        def compute_rank_metrics(pairs: tuple[torch.Tensor, torch.Tensor], save_path: Optional[str] = None):
            ...
            if getattr(model, "decoder_type", None) == "cosine":
                ...
                for i_row, ph_global in enumerate(ph_indices):
                    ...
                    # 屏蔽该 phage 的所有其它真 host（train∪val∪test）  先注释
                    # mask_set = set(global_pos_map.get(ph_global, set())) - set(ph2hosts[ph_global])
                    # for hpos in mask_set:
                    #     if 0 <= hpos < row.shape[0]:
                    #         row[hpos] = -np.inf
                ...
                for ph_idx, true_ds in ph2hosts.items():
                    ...
                    row = scores_np_all.copy()
                    # mask_set = set(global_pos_map.get(ph_idx, set())) - set(true_ds)   先注释
                    # for hpos in mask_set:
                    #     if 0 <= hpos < row.shape[0]:
                    #         row[hpos] = -np.inf
...
def softmax_ce_loss0(
    ...
    *,
    # —— 强推：归一化 + 学习式温度（更稳、更尖锐）——
    normalize: bool = True,
    logit_scale: Optional[torch.Tensor] = 1,  # e.g. model.logit_scale（标量参数）
    logit_scale_clamp: float = 5.0,              # 将 logit_scale 限制到 [-5, 5]（即 ~[e^-5, e^5]）
    # —— 可选：margin（提升 Top-1 间隔）——
    margin_type: Optional[str] = None,           # {"cosface","arcface",None}
    margin_m: float = 0.2,
    # —— 可选：多正样本（如同物种多个 GCF）——
    # 传入一个 dict: i -> [label_j1, label_j2, ...]，i 是正样本对的索引（对应 pos_* 张量的行）
    multi_pos: Optional[Dict[int, List[int]]] = None,
    label_smoothing: float = 0.0,                # 0.0~0.1，低成本防过拟合
    # —— 可选：对称项（host->phage 方向也做一次 CE）——
    symmetric: bool = False
    ) -> torch.Tensor:
    """
    改良的 in-batch softmax CE：
      - 余弦相似 + 学习式温度（或 tau）
      - 可选 CosFace / ArcFace margin（显著提升 Top-1）
      - 可选多正样本监督（同物种多个 host 都算正）
      - 可选对称项（再做一次 host->phage 的 CE）
    兼容你当前的调用：不传可选参数时≈原行为。
    """
    device = phage_emb_batch.device
    pos_phage_local_idx = pos_phage_local_idx.to(device)
    pos_host_local_idx  = pos_host_local_idx.to(device)

    # 1) 选取参与监督的 phage 子集（N_pos, D）
    q = phage_emb_batch.index_select(0, pos_phage_local_idx)
    k = host_emb_batch  # (H, D)

    # 2) 归一化（强烈建议打开，配合余弦稳定、可解释）
    if normalize:
        q = F.normalize(q, dim=1)
        k = F.normalize(k, dim=1)

    # 3) 计算 logits 并施加温度
    #    - 用学习式温度更灵活；否则退化为 1/tau
    if logit_scale is not None:
        # 将标量参数 clamp（避免过大/过小导致数值不稳）
        scale = torch.exp(torch.clamp(logit_scale, min=-logit_scale_clamp, max=logit_scale_clamp))
    ...
    # 之前算好的 logits_base（matmul + 温度）
    logits_base = logits  # 仅重命名强调“基准只读”

    y = pos_host_local_idx.long().view(-1, 1)

    if margin_type is not None and margin_type.lower() in {"cosface", "arcface"}:
        target_logit = logits_base.gather(1, y)  # (N_pos,1)

        if margin_type.lower() == "cosface":
            target_logit_m = target_logit - margin_m
        else:  # arcface
            cos_t  = torch.clamp(target_logit, -1.0, 1.0)
            theta  = torch.acos(cos_t)
            target_logit_m = torch.cos(theta + margin_m)

        # 非就地替换：安全
        logits = logits_base.scatter(1, y, target_logit_m)
    else:
        logits = logits_base


    # 5) 多正样本/标签平滑：将 one-hot 换成 soft target 分布
    if multi_pos is not None or (label_smoothing > 1e-8):
        N, C = logits.size()
        # soft target 初始化为全 0
        target = logits.new_zeros((N, C))

        # 逐样本填充正类
        for i in range(N):
            # 当前样本的主正类
            cj = int(pos_host_local_idx[i].item())
            if multi_pos is not None and i in multi_pos and len(multi_pos[i]) > 0:
                pos_list = list(set([cj] + [int(x) for x in multi_pos[i]]))
            else:
                pos_list = [cj]
            target[i, pos_list] = 1.0 / float(len(pos_list))  # 均分权重

        # 标签平滑
        if label_smoothing > 1e-8:
            target = (1 - label_smoothing) * target + (label_smoothing / C)

        # 用 KLDivLoss / CE with soft labels：log_softmax + (-sum p*log q)
        log_prob = F.log_softmax(logits, dim=1)
        loss_main = torch.sum(-target * log_prob, dim=1).mean()
    else:
        # 标准 hard label 交叉熵
        y = pos_host_local_idx.long()
        loss_main = F.cross_entropy(logits, y, reduction='mean')

    if not symmetric:
        return loss_main

    # 6) 可选对称项：host->phage 方向也做一遍 CE（让两侧空间等价）
    #    对称项通常能带来小幅稳定提升，有时对 Hit@1 也有帮助
    #    注意：这里用“相同的配对”反向监督
    # 构造 host 子集（N_pos, D），并用所有 phage 作为“类别”
    q2 = host_emb_batch.index_select(0, pos_host_local_idx)
    k2 = phage_emb_batch
    if normalize:
    ...
    if logit_scale is not None:
        scale = torch.exp(torch.clamp(logit_scale, min=-logit_scale_clamp, max=logit_scale_clamp))
        logits2 = torch.matmul(q2, k2.t()) * scale
    else:
        logits2 = torch.matmul(q2, k2.t())
        if tau != 1.0 and tau > 0.0:
            logits2 = logits2 / float(tau)

    # host->phage 的标签：就是 pos_phage_local_idx 对齐到 q2 的行
    y2 = pos_phage_local_idx.long()

    if multi_pos is not None or (label_smoothing > 1e-8):
        N2, C2 = logits2.size()
        target2 = logits2.new_zeros((N2, C2))
        for i in range(N2):
            ci = int(pos_phage_local_idx[i].item())
            # 反向这边通常没有“多正 phage”的需求，保留单一标签即可
            target2[i, ci] = 1.0

        if label_smoothing > 1e-8:
            target2 = (1 - label_smoothing) * target2 + (label_smoothing / C2)

        log_prob2 = F.log_softmax(logits2, dim=1)
        loss_sym = torch.sum(-target2 * log_prob2, dim=1).mean()
    else:
        loss_sym = F.cross_entropy(logits2, y2, reduction='mean')

    return 0.5 * (loss_main + loss_sym)
def main():
    ...
    edge_type_weight_map = {
        ...
        ('host', 'belongs_to', 'taxonomy'): 10.0,
        ('taxonomy', 'related', 'taxonomy'): 10.0,
    ...
    model = GATv2MiniModel(metadata=data.metadata(), in_dims=in_dims,
                         hidden_dim=args.hidden_dim, out_dim=args.out_dim,
                         n_layers=args.n_layers, n_heads=args.n_heads, dropout=args.dropout, decoder="cosine",use_edge_attr=True,edge_attr_dim=1,rel_init_map=edge_type_weight_map,).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            #     phage_emb_batch, host_emb_batch,
            #     pos_src_local, pos_dst_local,
            #     # tau 可忽略（传了 logit_scale 优先生效）
            #     normalize=True,
            #     logit_scale=model.logit_scale,     # 学习式温度
            #     logit_scale_clamp=5.0,             # 安全范围 exp(±5) ~ [0.007, 148]
            #     #margin_type="cosface",             # 推荐先用 cosface
            #     #margin_m=0.2,                      # 0.15~0.35 网格尝试
            #     #label_smoothing=0.05,              # 可先开 0.05
            #     symmetric=False                    # 可以先关，稳定后再开
            # )
        ...
        if epoch % args.log_every == 0 or epoch == args.epochs:
            try:
                ...
                save_predictions(model, data, test_src_cpu, test_dst_cpu, relation, eval_device, host_id2taxid, taxid2species, output_file=pred_file,k_list=(30,),edge_type_weight_map=edge_type_weight_map,node_maps_path=args.node_maps)
    ...
    save_predictions(model, data, test_src_cpu, test_dst_cpu, relation, eval_device, host_id2taxid, taxid2species, output_file=final_pred_file,k_list=(30,),edge_type_weight_map=edge_type_weight_map,node_maps_path=args.node_maps)
