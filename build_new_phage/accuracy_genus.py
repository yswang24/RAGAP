...
pred_df = pd.read_csv("newphage_predictions_cherry_2w_genus.tsv", sep="\t")
...
merged['correct'] = merged['host_genus_name'] == merged['genus']
...
hit5 = top5_df.groupby('phage_id')['host_genus_name'].apply(list).reset_index()
hit5 = pd.merge(hit5, true_df[['phage_id', 'genus']], on='phage_id', how='inner')
hit5['correct'] = hit5.apply(lambda row: row['genus'] in row['host_genus_name'], axis=1)
accuracy5 = hit5['correct'].mean()
print(f"Hit@5 accuracy: {accuracy5:.4f} ({hit5['correct'].sum()}/{len(hit5)})")

# ---- Hit@10 ----
top10_df = pred_df[pred_df['rank'] <= 10]
hit10 = top10_df.groupby('phage_id')['host_genus_name'].apply(list).reset_index()
hit10 = pd.merge(hit10, true_df[['phage_id', 'genus']], on='phage_id', how='inner')
hit10['correct'] = hit10.apply(lambda row: row['genus'] in row['host_genus_name'], axis=1)
...
if not wrong.empty:
    print("\n预测错误的 phage 列表:")
    print(wrong[['phage_id', 'host_genus_name', 'genus']].to_string(index=False))
else:
    print("\n所有预测都正确 ✅")
