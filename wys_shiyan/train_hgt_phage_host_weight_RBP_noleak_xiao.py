...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            # edge_attr 处理（和你原来完全一致）
            batch_edge_attr = {}
            for et in batch.edge_types:
                if hasattr(batch[et], 'edge_weight') and batch[et].edge_weight is not None:
                    v = batch[et].edge_weight.to(device)
                    batch_edge_attr[et] = v.view(-1, 1)
                elif et in edge_type_weight_map:
                    E = batch[et].edge_index.size(1)
                    batch_edge_attr[et] = torch.full((E,1), float(edge_type_weight_map[et]), device=device)
            edge_attr_arg = batch_edge_attr if batch_edge_attr else None

            out = model(x_dict, edge_index_dict, edge_attr_dict=edge_attr_arg)

            # === 正样本 ===
            pos_edge_label_index = batch[relation].edge_label_index  # (2, num_pos)
            if pos_edge_label_index.size(1) == 0:
                continue

            pos_src = pos_edge_label_index[0]
            pos_dst = pos_edge_label_index[1]
            pos_pred = model.decode(out, pos_edge_label_index, etype=relation)  # 必须实现 decode 返回标量分数

            # === 负采样（in-batch hard negative）===
            neg_ratio = args.neg_ratio  # 建议设 4~10
            phage_nids_local = torch.arange(out['phage'].size(0), device=device)
            host_nids_local  = torch.arange(out['host'].size(0),  device=device)

            # 对每个正样本 phage，随机采样 neg_ratio 个 host（过滤掉已有的训练正边，防止泄漏）
            # ———————————————————— 替换从 “# === 负采样” 开始的所有代码 ————————————————————
            # === 直接使用 in-batch 全负样本 + BPR Loss（最快、最稳定、效果最好）===
            pos_src_local = pos_edge_label_index[0]   # (num_pos,) local idx in current batch
            pos_dst_local = pos_edge_label_index[1]

            # 所有 phage 和 host 的 embedding（当前子图）
            phage_emb = out['phage']   # (n_phage_local, D)
            host_emb  = out['host']    # (n_host_local,  D)

            # 正样本分数
            pos_scores = model.decode(out, pos_edge_label_index, etype=relation)  # (num_pos,)

            # 负样本分数：每个正样本 phage，对当前 batch 内所有 host 打分（in-batch negatives）
            # 构造 (num_pos, n_host_local) 的 edge_label_index
            repeated_src = pos_src_local.unsqueeze(1).repeat(1, host_emb.size(0))        # (num_pos, n_host)
            all_dst      = torch.arange(host_emb.size(0), device=device).unsqueeze(0).repeat(pos_src_local.size(0), 1)

            neg_edge_index = torch.stack([repeated_src.flatten(), all_dst.flatten()])
            neg_scores = model.decode(out, neg_edge_index, etype=relation)               # (num_pos * n_host,)

            # 重要：把正样本自己的分数从负样本里去掉（避免负样本里混进正样本）
            mask = torch.ones(neg_scores.numel(), dtype=torch.bool, device=device)
            pos_indices_in_neg = pos_src_local * host_emb.size(0) + pos_dst_local
            mask[pos_indices_in_neg] = False
            neg_scores = neg_scores[mask]

            # BPR Loss（推荐！）
            loss = bpr_loss(pos_scores, neg_scores)
            # —————————————————————————————————————————————————————————————————————————————
            # 可选：pos 权重更高一点（处理正负不平衡）
            # pos_weight = torch.tensor([neg_ratio], device=device)
            # loss = torch.nn.functional.binary_cross_entropy_with_logits(
            #         pred_all, label_all, pos_weight=pos_weight)

            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=5.0)
            optimizer.step()

            # logit_scale / rel_logw clamp（保留你原来的稳定技巧）
            with torch.no_grad():
