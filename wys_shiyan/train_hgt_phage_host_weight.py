...
#         dropout: float = 0.2,
#         decoder: str = "mlp",
#         use_edge_attr: bool = True,     # <- 新增：是否启用 edge_attr/edge_weight 支持
...
# #class GATv2MiniModel(nn.Module)yuanlaihaoyong:
#     def __init__(
#         self,
#         metadata: tuple,                 # (node_types, edge_types)
#         in_dims: dict,                   # {ntype: in_dim}
...
#                 # IMPORTANT: pass as *_dict (HeteroConv requires that)
#                 # HeteroConv will forward processed[etype] to the child conv as the argument named `edge_attr`
...
def compute_metrics_fullgraph(
    ...
    # ======================
    try:
        with open("node_maps_cluster_650.json", "r", encoding="utf-8") as f:
            node_maps = json.load(f)
        phage_map = node_maps.get("phage_map", {})
        host_map = node_maps.get("host_map", {})
        phage_idx2id = {int(v): str(k) for k, v in phage_map.items()}
...
def save_predictions(
    ...
    try:
        ...
        # Load node maps for ID reversal
        try:
            with open("node_maps_RBP_650.json", "r", encoding="utf-8") as f:
                node_maps = json.load(f)
            phage_map = node_maps.get("phage_map", {})
            host_map = node_maps.get("host_map", {})
            phage_idx2id = {int(v): str(k) for k, v in phage_map.items()}
