...
def softmax_ce_loss(phage_emb_batch: torch.Tensor,
                    ...
                    pos_phage_local_idx: torch.LongTensor,
                    pos_host_local_idx: torch.LongTensor,
                    tau: float = 1.0,  # 默认设为 1，或者干脆不传
                    logit_scale: torch.Tensor = None): # 必须传这个
    ...
    # 2. 计算相似度矩阵 (N_pos, H_batch)
    # 此时 logits 的范围是 [-1, 1]
    logits = torch.matmul(phage_vecs, host_emb_batch.t()) 
    
    # 3. 关键修改：使用可学习的 logit_scale 进行放大
    if logit_scale is not None:
        # 限制一下最大值(防止溢出)，比如 max ln(100) = 4.6
        # clamp 是为了训练稳定性，防止初期梯度爆炸
        scale = torch.clamp(logit_scale, max=4.6).exp() 
        logits = logits * scale
    else:
        # 如果没传可学习参数，才使用固定的 tau
        logits = logits / tau
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # 7. 更新学习率 (注意：缩进要和 for batch 对齐，在 Epoch 结束时调用)
        scheduler.step()  # <--- 正确位置：每一个 Epoch 走一步
        t1 = time.time()
