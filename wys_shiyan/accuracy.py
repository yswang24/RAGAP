...
def main(args):
    ...
    # --- Modified summary: one row per k, columns for species/genus (rate & count) ---
    summary_k_rows = []
    if denom == 0:
        print("Warning: no phage with known true labels found in truth file. Summary will be zeros.")
    for k in ks:
    ...
    # format floats nicely when printing
...
Usage:
    python accuracy.py --pred /home/wangjingyuan/wys/wys_shiyan/output-train-GAT_softce_cluster_hid512_2layer_4heads_20.10_1024_neg70_evl20_drop0.2_1e-5_cos_new_40000_True_noleak_study2/predictions/phage_prediction_results_test_topk.tsv\
