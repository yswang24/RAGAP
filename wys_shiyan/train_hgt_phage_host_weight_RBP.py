...
class GATv2MiniModel(nn.Module):
    def __init__(
        ...
        dropout: float = 0.2,
        decoder: str = "mlp",
        use_edge_attr: bool = False,      # 是否启用 edge_attr 支持
...
def main():
    ...
    model = GATv2MiniModel(metadata=data.metadata(), in_dims=in_dims,
                         hidden_dim=args.hidden_dim, out_dim=args.out_dim,
                         n_layers=args.n_layers, n_heads=args.n_heads, dropout=args.dropout, decoder="cosine",use_edge_attr=False,edge_attr_dim=1).to(device)
        ...
        if epoch % args.log_every == 0 or epoch == args.epochs:
            try:
                ...
                save_predictions(model, data, test_src_cpu, test_dst_cpu, relation, eval_device,
                                ...
                                output_file=pred_file, k_list=(10,), edge_type_weight_map=edge_type_weight_map)
                ...
                save_predictions(model, data, test_src_cpu, test_dst_cpu, relation, eval_device, host_id2taxid, taxid2species, output_file=pred_file,k_list=(10,),edge_type_weight_map=edge_type_weight_map,node_maps_path=args.node_maps)
    ...
    train_metrics, val_metrics, test_metrics = compute_metrics_fullgraph(
        ...
        eval_device=args.eval_device, eval_neg_ratio=args.eval_neg_ratio,
        host_id2taxid=host_id2taxid, taxid2species=taxid2species,k_list=(1, 5, 10),   # <- 这里加上
        save_path=os.path.join(preds_dir, "phage_prediction_results"),
    ...
    save_predictions(model, data, test_src_cpu, test_dst_cpu, relation, eval_device, host_id2taxid, taxid2species, output_file=final_pred_file,k_list=(10,),edge_type_weight_map=edge_type_weight_map,node_maps_path=args.node_maps)
