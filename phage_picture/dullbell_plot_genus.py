...
data = {
    ...
    'Hit_1': [0.745, 0.375, 0.718, 0.670, 0.901],
...
ax.set_xlim(0, 1.0)
ax.set_title('Top-1 Accuracy Comparison (Genus level)', pad=12, fontweight='bold')
...
# === Annotate improvement between iPHoP and Ours ===
iphop_val = df.loc[df['Method'] == 'iPHoP', 'Hit_1'].values[0]
ours_val = df.loc[df['Method'] == 'Ours', 'Hit_1'].values[0]

y_iphop = int(df.index[df['Method'] == 'iPHoP'][0])
y_ours = int(df.index[df['Method'] == 'Ours'][0])

mid_x = (iphop_val + ours_val) / 2
annot_y = min(y_iphop, y_ours) - 0.45  # axis inverted

# bracket-style connector
ax.plot(
    [iphop_val, iphop_val, ours_val, ours_val],
    [annot_y + 0.15, annot_y, annot_y, annot_y + 0.15],
    color=color_map['Ours'], linewidth=1.6, solid_capstyle='butt', zorder=4
)

# relative improvement
impr_pct = (ours_val - iphop_val) / iphop_val * 100
ax.text(
    mid_x, annot_y - 0.06,
    f'+{15.6}% improvement',
...
# Save as SVG
outpath = 'dullbell_top1_comparison_genus.svg'
